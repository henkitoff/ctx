...
## Config Schema (simplified)  <!-- all-tiers -->
...
- [[modules/secrets.ctx|secrets]] — SecretRef encryption and resolution
- [[modules/infra.ctx|infra]] — file I/O utilities used by config loader
- [[modules/agents.ctx|agents]] — model catalog, auth profiles, session overrides
- [[modules/plugins.ctx|plugins]] — plugin config parsing (delegated to plugins)
- [[modules/channels.ctx|channels]] — channel config parsing
- [[modules/gateway.ctx|gateway]] — gateway config (port, TLS, auth mode)
- [[modules/cli.ctx|cli]] — config path resolution at startup
