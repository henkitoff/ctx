...
- `agent:bootstrap` — runs while building bootstrap files before system prompt
- Command hooks — `/new`, `/reset`, `/stop`, and other command events

### Plugin hooks (in-loop)
| Hook | When it fires |
|------|--------------|
| `before_model_resolve` | Pre-session, no messages — override provider/model deterministically |
| `before_prompt_build` | After session load, with messages — inject context or system prompt |
| `before_agent_reply` | After inline actions, before LLM call — can claim turn and return synthetic reply |
| `agent_end` | After completion — inspect final message list + metadata |
...
| `before_install` | Inspect scan findings before skill/plugin installs |

**Hook decision rules:**
- `before_tool_call: { block: true }` is terminal — stops lower-priority handlers
- `message_sending: { cancel: true }` is terminal — stops lower-priority handlers
- `{ block: false }` and `{ cancel: false }` are no-ops; they do NOT clear a prior block/cancel
...
## Session Persistence  <!-- opus-only -->

Two layers:

1. **`sessions.json`** — session store: `sessionKey → SessionEntry` (small, mutable)
   - Tracks: `sessionId`, `updatedAt`, `chatType`, model overrides, token counters, `compactionCount`, `memoryFlushAt`
   - Session reset triggers: `/new`, `/reset`, daily reset (4:00 AM gateway host), idle expiry
2. **`<sessionId>.jsonl`** — append-only transcript with tree structure (`id` + `parentId`)
   - Entry types: `message`, `compaction`, `branch_summary`, `custom_message`, `custom`
   - Tool pairs (call + result) kept together across compaction chunk boundaries
