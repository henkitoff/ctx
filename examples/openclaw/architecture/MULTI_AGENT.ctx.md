...
## Multiple Channel Accounts  <!-- all-tiers -->
...
## Cross-Agent Memory Search  <!-- sonnet+ -->
...
## Agent-to-Agent Messaging  <!-- sonnet+ -->
...
## Routing Algorithm Detail  <!-- opus-only -->

The Gateway evaluates bindings left-to-right within each priority tier.
If multiple bindings match at the same tier, the **first in config order wins**.

Important nuances:
- A binding omitting `accountId` matches the **default account only** — not all accounts
- Use `accountId: "*"` for channel-wide fallback across ALL accounts
- If you add the same channel binding for the same agent with explicit `accountId`, OpenClaw upgrades the existing channel-only binding (no duplicate created)
- Direct chats collapse to `agent:<agentId>:<mainKey>` — true per-person isolation requires one agent per person
- Thread binding: when a sub-agent session is bound to a Discord thread, follow-up messages in that thread keep routing to the same sub-agent session
